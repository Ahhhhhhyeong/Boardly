...
1. Supports email/password signup and signin, including automatic login after email confirmation.

1. 보드 생성, 수정, 삭제가 가능하며 각 보드에는 기본 Kanban 컬럼이 자동 생성됩니다.
1. Users can create, update, and delete boards, and each new board automatically receives default Kanban columns.

1. 컬럼과 카드를 추가, 수정, 삭제할 수 있으며 카드에는 우선순위, 진행률, 시작일, 마감일을 기록할 수 있습니다.
1. Users can add, update, and delete columns and cards. Cards can include priority, progress, start date, and deadline values.

1. 로그인 세션이 유지되는 경우 `/` 또는 `/auth` 접근 시 자동으로 대시보드로 이동합니다.
1. When a login session is active, visiting `/` or `/auth` automatically redirects to the dashboard.
...
1. Local development: Docker Compose 또는 개별 npm 실행을 사용할 수 있습니다.
...
1. `frontend` contains screens, routing, API calls, and UI components.

1. `backend`는 인증, 보드, 컬럼, 카드 API를 제공합니다.
1. `backend` provides authentication, board, column, and card APIs.

1. `docs/supabase_schema.md`는 Supabase 테이블과 RLS 정책 설정에 필요한 SQL을 포함합니다.
1. `docs/supabase_schema.md` includes the SQL required for Supabase tables and RLS policies.
...
1. Docker Desktop or Docker Engine is required when running with Docker.

1. Supabase 프로젝트를 생성하고 `docs/supabase_schema.md`의 SQL을 Supabase SQL Editor에서 실행해야 합니다.
1. Create a Supabase project and run the SQL from `docs/supabase_schema.md` in the Supabase SQL Editor.

1. Supabase Authentication의 Redirect URLs에 개발 환경 콜백 URL을 추가해야 합니다.
1. Add the development callback URL to Supabase Authentication Redirect URLs.
...
1. Create a `.env` file in the root directory using `.env.example` as a reference.
...
1. Set `SUPABASE_URL` to your Supabase project URL. The backend normalizes the value even if `/rest/v1` is included.

1. `SUPABASE_KEY`는 Supabase anon/publishable key를 입력합니다.
1. Set `SUPABASE_KEY` to your Supabase anon or publishable key.

1. 배포 환경에서는 `AUTH_REDIRECT_URL`을 실제 프론트엔드 도메인의 `/auth/callback` 주소로 변경합니다.
1. In production, set `AUTH_REDIRECT_URL` to the `/auth/callback` URL on your real frontend domain.
...
1. The frontend runs at `http://localhost:5173` by default.

1. 백엔드는 기본적으로 `http://localhost:3000`에서 실행됩니다.
1. The backend runs at `http://localhost:3000` by default.
...
1. When a user signs up, Supabase sends an email confirmation message.

1. 사용자가 메일의 확인 링크를 클릭하면 `/auth/callback`으로 돌아옵니다.
1. When the user clicks the confirmation link, the browser returns to `/auth/callback`.

1. 프론트엔드는 콜백 URL의 인증 토큰을 백엔드 `/auth/session`으로 전달합니다.
1. The frontend sends the auth token from the callback URL to backend `/auth/session`.

1. 백엔드는 토큰을 검증하고 HttpOnly 세션 쿠키를 설정한 뒤 사용자를 대시보드로 이동시킵니다.
1. The backend validates the token, sets an HttpOnly session cookie, and the app moves the user to the dashboard.
...
1. Supabase data is organized into `profiles`, `boards`, `columns`, and `cards` tables.

1. `profiles`는 Supabase `auth.users`와 1:1로 연결됩니다.
1. `profiles` has a one-to-one relationship with Supabase `auth.users`.

1. `boards`는 사용자별 보드를 저장합니다.
1. `boards` stores each user's boards.

1. `columns`는 보드의 Kanban 단계 정보를 저장합니다.
1. `columns` stores Kanban workflow stages for each board.

1. `cards`는 각 보드와 컬럼에 속한 작업 항목을 저장합니다.
1. `cards` stores task items that belong to a board and column.

## 11. API 개요

1. 인증 API는 `/auth/signup`, `/auth/signin`, `/auth/session`, `/auth/signout`, `/auth/me`를 제공합니다.
1. Auth APIs include `/auth/signup`, `/auth/signin`, `/auth/session`, `/auth/signout`, and `/auth/me`.

1. 보드 API는 `/boards`에서 조회와 생성을 처리하고, `/boards/:id`에서 수정과 삭제를 처리합니다.
1. Board APIs use `/boards` for listing and creation, and `/boards/:id` for update and deletion.

1. 컬럼 API는 `/columns`와 `/columns/:id`를 통해 컬럼 데이터를 관리합니다.
1. Column APIs manage column data through `/columns` and `/columns/:id`.

1. 카드 API는 `/cards`와 `/cards/:id`를 통해 카드 데이터를 관리합니다.
1. Card APIs manage card data through `/cards` and `/cards/:id`.
...
1. The backend can run in a Node.js environment with `npm start`.
...
1. See `docs/supabase_schema.md` for the Supabase schema and RLS policies.

1. 프론트엔드 원본 README는 `frontend/README.md`에 있습니다.
1. The original frontend README is available at `frontend/README.md`.
